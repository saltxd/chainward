...
```bash
ssh saltxd@192.168.1.230 'kubectl get secret -n chainward chainward-secrets -o jsonpath="{.data.BASE_RPC_URL}" | base64 -d'
```
...
```bash
ssh saltxd@192.168.1.230 'kubectl patch secret -n chainward chainward-secrets --type=json -p="[
...
```bash
ssh saltxd@192.168.1.230 'kubectl logs -n chainward <indexer-pod> --tail=30 -f'
```
