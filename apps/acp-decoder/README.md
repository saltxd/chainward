...
| **Marketplace visibility** | ✅ **FIXED + root-caused (2026-08-20, revising the same-day "broken" finding).** Three stacked artifacts made the listing look absent: (1) `acpx.virtuals.io/api/agents` is the *legacy/web* index — the v2 agent-buyer surface is `acp browse` against `api.acp.virtuals.io`; (2) browse excludes *your own agent* — searching as Chainward hid Chainward (test as a different agent, e.g. `chainward-decoder`); (3) the REAL defect: **the ACP socket zombies silently** — pod Running since Aug 7 but presence stale since Aug 17 → absent from every `--online` browse, i.e. invisible to buyers filtering for live sellers. Pod restart fixed it instantly: **rank #1 for "wallet decode" (online), both offerings visible.** Durable mitigation: `deploy/acp-decoder-presence-refresh.yaml` (12-hourly rollout-restart CronJob, applied). Soft-launch tweet posted 2026-08-20 (`2090431512764088420`). Remaining gap: not in the legacy web index (likely graduation-gated) — agent-to-agent buyers unaffected. |
